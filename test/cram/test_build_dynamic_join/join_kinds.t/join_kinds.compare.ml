module Sqlgg (T : Sqlgg_traits.M) = struct
  ...
  module Implicit_before_candidate_col = struct
    ...
    let select db (col : _ t) ~uid callback =
      ...
      in
      T.select db
      ("SELECT " ^ col.projection.column ^ " FROM users u JOIN shipments s USING (user_id)" ^ (if List.mem Profiles col.deps then " LEFT JOIN profiles p ON p.user_id = u.id" else "") ^ " WHERE u.id = ?")

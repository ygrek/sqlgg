...
let get_column_Int (row : mock_row_data) (index : int) : Types.Int.t = 
  ...
  | `UInt64 u ->
      let v = Unsigned.UInt64.to_int64 u in
      printf "[MOCK] get_column_Int[%d] = %Ld (from uint64)\n" index v; v
...
let get_column_UInt64 (row : mock_row_data) (index : int) : Unsigned.UInt64.t =
  ...
  | `Int v ->
      let v' = if v < 0L then 0L else v in
      let u = Unsigned.UInt64.of_int64 v' in
      printf "[MOCK] get_column_UInt64[%d] = %s (from int64)\n" index (Unsigned.UInt64.to_string u);
      u
...
let get_column_Int_nullable (row : mock_row_data) (index : int) : Types.Int.t option = 
  ...
  | `UInt64 u ->
      let v = Unsigned.UInt64.to_int64 u in
      printf "[MOCK] get_column_Int_nullable[%d] = Some %Ld (from uint64)\n" index v; Some v
...
let get_column_UInt64_nullable (row : mock_row_data) (index : int) : Unsigned.UInt64.t option =
  ...
  | `Int v ->
      let v' = if v < 0L then 0L else v in
      let u = Unsigned.UInt64.of_int64 v' in
      printf "[MOCK] get_column_UInt64_nullable[%d] = Some %s (from int64)\n" index (Unsigned.UInt64.to_string u);
      Some u

...
type result = M.Res.t
type execute_response = { affected_rows : int64; insert_id: int }
...
let execute db sql set_params =
  ...
  let open IO in
  set_params stmt >>=
  fun res -> return { affected_rows = Int64.of_int (M.Res.affected_rows res); insert_id = M.Res.insert_id res }

...
let rec get_params_q (e:expr_q) =
  let rec loop acc e =
    ...
    | `Value _ -> acc
    | `InChoice (p, b, e) -> ChoiceIn (p, b, get_params_q e) :: acc
...
let rec resolve_columns env expr =
  ...
  let rec each e =
    ...
    | Param x -> `Param x
    | Inparam x -> `Inparam x
    | InChoice (n, b, x) -> `InChoice (n, b, each x)
      ...
      let as_params p =
        List.map
          (function
            ...
            | ChoiceIn (p, _, _) -> failed ~at:p.pos "FIXME as_params in ChoiceIn"
...
and assign_types expr =
  ...
  let rec typeof (e:expr_q) = (* FIXME simplify *)
    ...
    | `InChoice (n, b, e) -> let e, t = typeof e in `InChoice (n, b, e), t
    | `Choice (n,l) ->
...
let unify_params l =
  ...
  | ChoiceIn (p, b, l) -> ChoiceIn (p, b, List.map map l)

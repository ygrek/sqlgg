...
let rec get_params_q (e:expr_q) =
  let rec loop acc e =
    ...
    | `Value _ -> acc
    | `InChoice (p, e) -> ChoiceIn (p, get_params_q e) :: acc
...
let rec resolve_columns env expr =
  ...
  let rec each e =
    ...
    | Param x -> `Param x
    | Inparam x -> `Inparam x
    | InChoice (n, x) -> `InChoice (n, each x)
      ...
      let as_params p =
        List.map
          (function
            ...
            | ChoiceIn (p, _) -> failed ~at:p.pos "FIXME as_params in ChoiceIn"
...
and assign_types expr =
  ...
  let rec typeof (e:expr_q) = (* FIXME simplify *)
    ...
    | `InChoice (n, e) -> let e, t = typeof e in `InChoice (n, e), t
    | `Choice (n,l) ->
...
and ensure_simple_expr = function
  ...
  | InChoice (p,_) -> failed ~at:p.pos "ensure_simple_expr InChoice TBD"
...
let unify_params l =
  ...
  | ChoiceIn (p,l) -> check_choice_name p; List.iter traverse l
  ...
  | ChoiceIn (p, l) -> ChoiceIn (p, List.map map l)

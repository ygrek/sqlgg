...
let exists_grouping columns =
  List.exists (function Expr ({ value; _ }, _) -> is_grouping value | All | AllOf _ -> false) columns

let exists_windowing columns =
  List.exists (function Expr ({ value; _ }, _) -> is_windowing value | All | AllOf _ -> false) columns  
...
let make_dynamic_select ~is_subquery columns =
  if !Config.dynamic_select && not is_subquery then
    let choices = columns |> List.filter_map (function
      | Expr ({ value = e; pos = (ep_start, ep_end) }, alias) ->
        ...
        in
        Some ({ value = Some col_name; pos = (ep_start, ep_end) }, Some e)
      | All | AllOf _ -> None
    ) in
    match choices with
    | [] -> columns
    | (({ pos = (first_pos, _); _ }), _) :: _ ->
      let (({ pos = (_, last_pos); _ } : param_id), _) = List.last choices in
      let pos = (first_pos, last_pos) in
      [Expr ({ value = Choices ({ value = Some "col"; pos }, choices); pos }, None)]
  else
    columns
  ...
  let rec each e =
    ...
    | SelectExpr (select, usage) ->
      ...
      | [ { attr = {domain; _}; _ } ], `AsValue -> 
        ...
        | ({ columns = [Expr({ value = c; _ }, _)]; _ }, _) -> c |> with_count |> Option.default default_null
...
(*   let all = tables |> List.map snd |> List.flatten in *)
  let rec propagate_meta ~env = function
    ...
    | SelectExpr ({ select_complete = { select = ({columns = [Expr ({ value; _ }, _)]; from; _}, _); _ }; _ }, _) ->
...
and get_params_of_columns env =
  let get = function
  | All | AllOf _ -> []
  | Expr ({ value = Choices (p, choices); _ }, _) when not env.is_subquery && !Config.dynamic_select ->
    [DynamicSelect (p, List.map (fun (n, e) -> 
      Simple (n, Option.map (fun e -> e |> resolve_types env |> fst |> get_params_of_res_expr env) e)
    ) choices)]
  | Expr ({ value; _ }, _) -> get_params env value
  in
  flat_map get
...
and eval_select env { columns; from; where; group; having; } =
  ...
  let columns = make_dynamic_select ~is_subquery:env.is_subquery columns in
...
and resolve_source env (x, alias) =
  ...
  | `ValueRows { row_constructor_list; row_order; row_limit; } ->
    ...
    *)
    let exprs_to_cols = List.mapi (fun idx e -> Expr ({ value = e; pos = (0, 0) }, Some (Printf.sprintf "column_%d" idx))) in
...
let annotate_select select attrs =
  ...
  let apply_to_columns cols attrs =
    let rec loop acc cols attrs =
      ...
      | Expr (loc, name) :: cols, a :: attrs ->
        let e = merge_meta_into_params ~shallow:false a.meta loc.value in
        let t = a.domain in
        loop (Expr ({ loc with value = Fun { fn_name = "insert_select"; kind = (F (Typ t, [Typ t])); parameters = [e]; is_over_clause = false} }, name) :: acc) cols attrs

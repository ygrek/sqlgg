...
%{
  ...
  let mk_inline_index ~kind ~pos (idx_name, idx_cols) =
    `Index (make_located ~value:{ idx_kind = kind; idx_name; idx_cols; idx_unique = false } ~pos)
%}
...
column_def1: c=column_def { `Attr c }
           ...
           | kind=inline_idx_kind t=table_index { mk_inline_index ~kind ~pos:($startofs, $endofs) t }
